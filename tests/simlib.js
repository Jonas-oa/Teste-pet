...
const { getRelMin, addMin, recalc, reordenarFilaPorPosicao,
        pacParaLinha, linhaParaPac, instantePac, difPac, COL_PAC, mesclarNaCarga, horariosDuplicados, stableStringify } = N;
...
class Servidor {
  ...
  upsertOperacao(row){
    this.ops++;
    const normalizada={...row};
    if(normalizada.cfg) normalizada.cfg=JSON.parse(stableStringify(normalizada.cfg));
    this.operacao[row.sala_id]={...(this.operacao[row.sala_id]||{sala_id:row.sala_id,cfg:{}}),...normalizada};
...
class Cliente {
  ...
  _cargaInicial(modo){
    ...
    if(op&&op.cfg&&Object.keys(op.cfg).length){
      this.ecoCfg=stableStringify(op.cfg);
      this.cfg={...this.cfg,...op.cfg};
    }else if(modo==='enviar'){
      this.ecoCfg=stableStringify(this.cfg);
  ...
  _efeitoPushCfg(){
    if(!this.sala||!this.pronto||!this.online) return false;
    const s=stableStringify(this.cfg);
